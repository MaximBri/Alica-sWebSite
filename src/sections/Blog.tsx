...
export const Blog = () => {
  ...
  return (
    <section id='blog' className="flex flex-col items-center pt-10 pb-60 md:py-20 lg:py-100">
      ...
      {countItems < blogList.length && (
        <button className="gradient-border-button-bg border-0 text-white rounded-[5px] z-10 mt-10 lg:mt-60 relative px-10 py-[15px] lg:px-[56px] lg:py-[19px]" onClick={addCountItems}>
          {theme.theme === 'light' ? (
            <span className="rounded-10  bg-viola dark:bg-viola-dark z-0">
              <p className="button-text font-semibold text-base leading-1.1 bg-switch dark:bg-white z-20">More</p>
            </span>
          ) : (
            <p className="button-text font-semibold text-base leading-1.1 dark:bg-white z-20">More</p>
          )}
        </button>
      )}
    </section>
